...
GPT_DIRECT_MODE_ERROR = "GPT models are handled by sub2api direct mode, not PaperBanana."


def is_gpt_model_name(model_name: str) -> bool:
    normalized = (model_name or "").strip().lower()
    for prefix in ("openrouter/openai/", "openai/"):
...
async def call_model_with_retry_async(
    ...
    if is_gpt_model_name(model_name):
    ...
    # Convert Gemini GenerateContentConfig -> dict for Claude/OpenRouter.
    cfg_dict = {
