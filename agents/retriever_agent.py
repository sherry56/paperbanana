...
def _retriever_env_int(name: str, default: int) -> int:
...
def _truncate_field(text: str, max_chars: int, label: str) -> str:
    s = str(text) if text is not None else ""
    if len(s) <= max_chars:
        return s
    print(
        f"[Retriever] Truncated {label} from {len(s)} to {max_chars} chars "
        f"(keeps prompts under provider token limits, e.g. OpenRouter 600k)."
    )
    return s[:max_chars] + "\n\n[... truncated ...]"
...
class RetrieverAgent(BaseAgent):
    ...
    def __init__(self, **kwargs):
        ...
        if self.exp_config.task_name == "plot":
            ...
            # Default 100 (+ per-field truncation) avoids OpenRouter's ~600k prompt cap when
            # using auto retrieval; override with PAPERBANANA_RETRIEVER_DIAGRAM_REF_LIMIT=200, etc.
            self.task_config = {
                "task_name": "diagram",
                "ref_limit": _retriever_env_int("PAPERBANANA_RETRIEVER_DIAGRAM_REF_LIMIT", 100),
    ...
    async def _retrieve_and_parse(self, data: Dict[str, Any], cfg: dict) -> list:
        ...
        max_target = _retriever_env_int("PAPERBANANA_RETRIEVER_MAX_TARGET_CHARS", 24000)
        max_field = _retriever_env_int("PAPERBANANA_RETRIEVER_MAX_CANDIDATE_FIELD_CHARS", 4000)

        content = _truncate_field(str(data["content"]), max_target, "target methodology/content")
        visual_intent = _truncate_field(str(data["visual_intent"]), max_target, "target visual_intent")
        ...
        with open(self.exp_config.work_dir / f"data/PaperBananaBench/{cfg['task_name']}/ref.json", "r", encoding="utf-8") as f:
            ...
            limit = cfg["ref_limit"]
            if limit is None:
                limit = _retriever_env_int("PAPERBANANA_RETRIEVER_PLOT_REF_LIMIT", 100)
            candidate_pool = candidate_pool[:limit]
        
        for idx, item in enumerate(candidate_pool):
            c_id = item.get("id", "")
            c_vi = _truncate_field(str(item.get("visual_intent", "")), max_field, f"candidate {idx+1} visual_intent")
            c_body = _truncate_field(str(item.get("content", "")), max_field, f"candidate {idx+1} content")
            user_prompt += f"Candidate {cfg['candidate_type']} {idx+1}:\n"
            user_prompt += f"- {cfg['candidate_labels'][0]}: {c_id}\n"
            user_prompt += f"- {cfg['candidate_labels'][1]}: {c_vi}\n"
            user_prompt += f"- {cfg['candidate_labels'][2]}: {c_body}\n\n"
